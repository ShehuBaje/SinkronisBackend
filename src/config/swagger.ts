...
const options: swaggerJSDoc.Options = {
  definition: {
    ...
    paths: {
      ...
      [`${env.API_PREFIX}/payroll/pay-runs`]: { get: { tags: ["Payroll"], summary: "List tenant pay runs", description: "PENDING_APPROVAL returns REVIEW; historical states return VIEW.", security: [{ bearerAuth: [] }], parameters: [{ in: "query", name: "page", schema: { type: "integer", minimum: 1 } }, { in: "query", name: "limit", schema: { type: "integer", maximum: 100 } }, { in: "query", name: "search", schema: { type: "string" } }, { in: "query", name: "status", schema: { $ref: "#/components/schemas/PayrollPayRunStatus" } }, { in: "query", name: "sortBy", schema: { type: "string", enum: ["period", "createdAt", "employees", "gross", "netPay", "paye", "status"] } }, { in: "query", name: "sortOrder", schema: { type: "string", enum: ["asc", "desc"] } }], responses: { "200": { description: "Paginated pay runs" }, "400": { description: "Invalid filter" }, "401": { description: "Authentication required" }, "403": { description: "payroll:runs:view required" } } }, post: { tags: ["Payroll"], summary: "Compute and preview a payroll run", description: "Atomically selects eligible enrolled employees, snapshots calculation inputs/results, persists reconciled totals, and moves the run to PENDING_APPROVAL. It does not disburse funds or include Payees in the employee count.", security: [{ bearerAuth: [] }], requestBody: { required: true, content: { "application/json": { schema: { $ref: "#/components/schemas/PayrollPayRunCreateRequest" } } } }, responses: { "201": { description: "Computed payroll preview" }, "400": { description: "Invalid period or no eligible employees" }, "401": { description: "Authentication required" }, "403": { description: "payroll:runs:create required" }, "409": { description: "Overlapping pay run" } } } },
      [`${env.API_PREFIX}/payroll/pay-runs/{payRunId}`]: { get: { tags: ["Payroll"], summary: "Get computed pay run detail", description: "Returns persisted immutable employee snapshot rows and reconciled PayRun totals. Search matches employee name, employee number, or department.", security: [{ bearerAuth: [] }], parameters: [{ in: "path", name: "payRunId", required: true, schema: { type: "string" } }, { in: "query", name: "page", schema: { type: "integer" } }, { in: "query", name: "limit", schema: { type: "integer", maximum: 100 } }, { in: "query", name: "search", schema: { type: "string" } }], responses: { "200": { description: "Pay run and employee items" }, "401": { description: "Authentication required" }, "403": { description: "payroll:runs:view required" }, "404": { description: "Tenant-owned run not found" } } } },
      ...
      [`${env.API_PREFIX}/payroll/wallet/obligations`]: { get: { tags: ["Payroll Wallet"], summary: "List payable payroll and statutory obligations", security: [{ bearerAuth: [] }], responses: { "200": { description: "Authoritative pending obligations" } } } },
