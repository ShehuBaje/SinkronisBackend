...
const options: swaggerJSDoc.Options = {
  definition: {
    ...
    paths: {
      ...
      [`${env.API_PREFIX}/accounting/items-services`]: { get: { tags: ["Accounting - Items & Services"], summary: "List catalogue items and services", security: [{ bearerAuth: [] }], parameters: [{ in: "query", name: "search", schema: { type: "string" } }, { in: "query", name: "type", schema: { type: "string", enum: ["ALL", "ITEM", "PRODUCT", "SERVICE"] } }, { in: "query", name: "status", schema: { type: "string", enum: ["ALL", "ACTIVE", "INACTIVE"] } }, { in: "query", name: "page", schema: { type: "integer" } }, { in: "query", name: "limit", schema: { type: "integer", maximum: 100 } }], responses: { "200": { description: "Catalogue and pagination" } } }, post: { tags: ["Accounting - Items & Services"], summary: "Create catalogue item/service", description: "vatApplicable selects the central invoice tax behavior; no display percentage is persisted.", security: [{ bearerAuth: [] }], responses: { "201": { description: "Created" }, "400": { description: "Invalid type, price or unit" } } } },
      ...
      [`${env.API_PREFIX}/accounting/projects`]: { get: { tags: ["Accounting - Projects"], summary: "List projects", security: [{ bearerAuth: [] }], parameters: [{ in: "query", name: "search", schema: { type: "string" } }, { in: "query", name: "status", schema: { type: "string", enum: ["ALL", "ACTIVE", "ON_HOLD", "COMPLETED"] } }, { in: "query", name: "page", schema: { type: "integer" } }, { in: "query", name: "limit", schema: { type: "integer", maximum: 100 } }, { in: "query", name: "sortBy", schema: { type: "string", enum: ["createdAt", "name", "value", "startDate", "endDate", "status"] } }, { in: "query", name: "sortOrder", schema: { type: "string", enum: ["asc", "desc"] } }], responses: { "200": { description: "Summary, projects and pagination" } } }, post: { tags: ["Accounting - Projects"], summary: "Create project", description: "Customer and optional accounting-enabled agent are tenant-validated.", security: [{ bearerAuth: [] }], responses: { "201": { description: "Created" }, "400": { description: "Invalid dates/value" }, "404": { description: "Customer or agent not found in tenant" } } } },
      ...
      [`${env.API_PREFIX}/accounting/invoices`]: { get: { tags: ["Accounting - Invoices"], summary: "List invoices", description: "Tenant-scoped search, client/agent/project/status/date filters and pagination. OVERDUE includes past-due unpaid SENT invoices. Responses include the frozen WHT selection, WHT amount and net amount payable.", security: [{ bearerAuth: [] }], parameters: ["search", "clientId", "agentId", "projectId", "status", "fromDate", "toDate", "page", "limit"].map((name) => ({ in: "query", name, schema: { type: name === "page" || name === "limit" ? "integer" : "string" } })), responses: { "200": { description: "Invoice rows and pagination" }, "401": { description: "Unauthenticated" }, "403": { description: "Accounting entitlement or permission unavailable" } } }, post: { tags: ["Accounting - Invoices"], summary: "Create draft invoice", description: "Backend resolves catalogue snapshots and authoritative VAT, then calculates subtotal, VAT and total. WHT is optional and manually selected per invoice at 5% or 10%; it is calculated on subtotal before VAT and deducted from total to produce amountPayable.", security: [{ bearerAuth: [] }], requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["clientId", "dueDate", "items"], properties: { clientId: { type: "string" }, assignedAgentId: { type: "string", nullable: true }, projectId: { type: "string", nullable: true }, dueDate: { type: "string", format: "date" }, notes: { type: "string" }, whtApplicable: { type: "boolean", default: false }, whtRate: { type: "integer", enum: [5, 10], description: "Required only when whtApplicable is true." }, items: { type: "array", minItems: 1, items: { type: "object", required: ["quantity"], properties: { catalogueItemId: { type: "string" }, description: { type: "string" }, quantity: { type: ["string", "number"] }, unitPrice: { type: ["string", "number"] }, vatApplicable: { type: "boolean" } } } } } } } } }, responses: { "201": { description: "Draft invoice with frozen lines, tax/WHT values and status history" }, "400": { description: "Validation error" }, "404": { description: "Tenant-owned relation not found" } } } },
