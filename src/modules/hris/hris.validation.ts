...
export const managerReviewSchema = z.object({ goalRatings: z.array(z.object({ goalId: id, rating: z.number().int().min(1).max(5), comment: z.string().trim().max(3000).optional() }).strict()).max(100).default([]), responses: z.array(reflectionSchema).max(100).default([]), overallFeedback: z.string().trim().min(3).max(10000).optional(), recommendation: z.enum(["ON_TRACK", "NEEDS_IMPROVEMENT", "EXCEEDS_EXPECTATION"]).optional(), submit: z.boolean().default(true) }).strict().superRefine((value, context) => {
  if (value.submit === false) return;
  if (!value.overallFeedback) context.addIssue({ code: z.ZodIssueCode.custom, message: "overallFeedback is required when submitting", path: ["overallFeedback"] });
  if (!value.recommendation) context.addIssue({ code: z.ZodIssueCode.custom, message: "recommendation is required when submitting", path: ["recommendation"] });
});
...
const templateQuestionSchema = z.object({ id: z.string().trim().min(1).max(191), question: z.string().trim().min(3).max(1000), required: z.boolean().default(true) }).strict();
