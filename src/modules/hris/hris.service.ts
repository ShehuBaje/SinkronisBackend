...
export const confirmEmployeeAppraisalGoals = async (organizationId: string, appraisalId: string, user: AuthUser) => {
  ...
  const now = new Date(); const changed = await prisma.employeeAppraisal.updateMany({ where: { id: appraisal.id, organizationId, employeeId: appraisal.employeeId, stage: "GOAL_SETTING", goalsConfirmedAt: null }, data: { goalsConfirmedAt: now, goalsConfirmedById: user.id, stage: "SELF_ASSESSMENT" } });
  if (changed.count !== 1) throw conflict("Appraisal goals were confirmed concurrently");
  await prisma.appraisalGoal.updateMany({ where: { organizationId, appraisalId }, data: { status: "LOCKED" } });
...
const hasMeaningfulSelfAssessmentInput = (sections: any[], reflections: any[]) => reflections.some((item) => item.response.trim().length > 0) || sections.some((section) => section.objectives.some((objective: any) => objective.keyResults.some((result: any) => typeof result.achieved === "number" || Object.values(result.achieved).some((value) => value != null))));
...
export const saveSelfAssessment = async (organizationId: string, appraisalId: string, input: any, user: AuthUser) => {
  ...
  const snapshot = appraisal.templateSnapshot ?? appraisal.cycle.templateSnapshot ?? appraisal.template.configuration; const authoritativeSections = appraisalAssessmentFromTemplate(snapshot, input.sections); const weighted = calculateWeightedAssessmentScore(authoritativeSections); const validQuestionIds = new Set(templateQuestions(snapshot).map((question: any) => String(question.id)));
  ...
  if (input.submit && !hasMeaningfulSelfAssessmentInput(authoritativeSections, input.reflections)) throw badRequest("Enter at least one achieved value or reflection answer before submitting");
