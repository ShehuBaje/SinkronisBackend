  ...
  const normalized = sections.map((section) => {
    ...
    return { ...section, objectives: section.objectives.map((objective: any) => {
      ...
      return { ...objective, keyResults: objective.keyResults.map((keyResult: any) => {
        ...
        const achieved = typeof keyResult.achieved === "number" ? keyResult.achieved : keyResult.achieved && typeof keyResult.achieved === "object" ? Number(Object.values(keyResult.achieved).find((value) => value != null) ?? 0) : 0;
...
export const getAppraisalDetail = async (organizationId: string, appraisalId: string, user: AuthUser) => {
  ...
  const snapshot = appraisal.templateSnapshot && typeof appraisal.templateSnapshot === "object" && !Array.isArray(appraisal.templateSnapshot) ? appraisal.templateSnapshot as Record<string, any> : {}; const templateMetadata = snapshot.templateMetadata && typeof snapshot.templateMetadata === "object" ? snapshot.templateMetadata : {};
  const isEmployee = access.self?.id === appraisal.employeeId;
  const reviewReleased = ["ACKNOWLEDGMENT", "COMPLETED"].includes(appraisal.stage) && appraisal.hrApproval?.decision === "APPROVED";
  const ratingScale = Array.isArray(snapshot.ratingScale) ? snapshot.ratingScale : Object.entries(ratingLabels).map(([value, label]) => ({ value: Number(value), label }));
  return { appraisalId: appraisal.id, employee: { id: appraisal.employee.id, employeeCode: appraisal.employee.employeeNo, name: `${appraisal.employee.firstName} ${appraisal.employee.lastName}`, role: appraisal.employee.jobTitle, department: appraisal.employee.department?.name ?? null, manager: appraisal.manager ? { id: appraisal.manager.id, name: `${appraisal.manager.firstName} ${appraisal.manager.lastName}`, role: appraisal.manager.jobTitle } : null, startDate: appraisal.employee.hireDate, profileImage: appraisal.employee.profileImageUrl ?? appraisal.employee.user?.profileImageUrl }, reviewer: appraisal.manager ? { id: appraisal.manager.id, name: `${appraisal.manager.firstName} ${appraisal.manager.lastName}`, role: appraisal.manager.jobTitle } : null, cycle: { id: appraisal.cycle.id, name: appraisal.cycle.title, periodFrom: appraisal.cycle.periodStart, periodTo: appraisal.cycle.periodEnd, submissionDeadline: appraisal.cycle.deadline ?? appraisal.cycle.periodEnd, quarter: appraisal.cycle.quarter, year: appraisal.cycle.year }, template: { id: appraisal.template.id, name: templateMetadata.name ?? appraisal.template.name, version: templateMetadata.version ?? null }, criteria: Array.isArray(snapshot.sections) ? snapshot.sections : [], ratingScale, stage: appraisal.stage, status: appraisal.status, goalsConfirmedAt: appraisal.goalsConfirmedAt, finalScore: appraisal.finalScore == null ? null : Number(appraisal.finalScore), ratingValue: appraisal.ratingValue, rating: appraisal.rating, workflow: { goalSetting: appraisal.stage === "GOAL_SETTING" ? "IN_PROGRESS" : "LOCKED", selfAssessment: appraisal.selfAssessment?.status ?? "NOT_STARTED", managerReview: appraisal.managerReview?.status ?? "NOT_STARTED", hrApproval: appraisal.hrApproval?.decision ?? "NOT_STARTED", acknowledgment: appraisal.acknowledgedAt ? "ACKNOWLEDGED" : "NOT_STARTED" }, workflowHistory: [{ stage: "GOAL_SETTING", completedAt: appraisal.goalsConfirmedAt }, { stage: "SELF_ASSESSMENT", completedAt: appraisal.selfAssessment?.submittedAt ?? null }, { stage: "MANAGER_REVIEW", completedAt: appraisal.managerReview?.submittedAt ?? null }, { stage: "HR_APPROVAL", completedAt: appraisal.hrApproval?.approvedAt ?? null }, { stage: "ACKNOWLEDGMENT", completedAt: appraisal.acknowledgedAt }], goals: appraisal.goals.map((goal) => ({ ...goal, employeeRatingLabel: goal.employeeRating ? ratingLabels[goal.employeeRating as keyof typeof ratingLabels] : null, managerRatingLabel: goal.managerRating ? ratingLabels[goal.managerRating as keyof typeof ratingLabels] : null })), selfAssessment: appraisal.selfAssessment, managerReview: !isEmployee || reviewReleased ? appraisal.managerReview : null, hrApproval: access.hr ? appraisal.hrApproval : appraisal.hrApproval ? { decision: appraisal.hrApproval.decision, approvedAt: appraisal.hrApproval.approvedAt } : null, acknowledgement: { acknowledged: Boolean(appraisal.acknowledgedAt), response: appraisal.acknowledgedResponse, acknowledgedAt: appraisal.acknowledgedAt }, acknowledgedResponse: appraisal.acknowledgedResponse, signOffs: appraisal.signOffs, timestamps: { createdAt: appraisal.createdAt, updatedAt: appraisal.updatedAt, selfAssessmentSubmittedAt: appraisal.selfAssessment?.submittedAt ?? null, managerReviewedAt: appraisal.managerReview?.submittedAt ?? null, hrApprovedAt: appraisal.hrApproval?.approvedAt ?? null, acknowledgedAt: appraisal.acknowledgedAt } };
};
export const createAppraisalGoal = async (organizationId: string, appraisalId: string, input: any, user: AuthUser) => {
  const appraisal = await getTenantAppraisal(organizationId, appraisalId); const self = await actorEmployee(organizationId, user.id); const isEmployee = self?.id === appraisal.employeeId; const isManager = canApproveAppraisals(user) || (self?.id === appraisal.managerId && canManageAppraisals(user));
  if (!isEmployee && !isManager) throw forbidden("Goal creation is not permitted");
  if (appraisal.stage !== "GOAL_SETTING" || appraisal.goalsConfirmedAt) throw conflict("Goal setting is locked for this appraisal");
  ...
  if (await prisma.appraisalGoal.findFirst({ where: { organizationId, appraisalId, title: input.goalTitle }, select: { id: true } })) throw conflict("An appraisal goal with this title already exists");
  ...
  const recipients = await userIdsForEmployees(organizationId, [isEmployee ? appraisal.managerId : appraisal.employeeId]);
  ...
  await createAuditLog({ organizationId, actorUserId: user.id, action: "HRIS_APPRAISAL_GOAL_PROPOSED", resource: "APPRAISAL", resourceId: appraisal.id, summary: `Proposed appraisal goal: ${goal.title}`, metadata: { employeeId: appraisal.employeeId, goalId: goal.id, creator: isEmployee ? "EMPLOYEE" : "MANAGER", notification } }); return goal;
};
...
export const openAppraisalSelfAssessment = async (organizationId: string, appraisalId: string, user: AuthUser) => { const appraisal = await getTenantAppraisal(organizationId, appraisalId); await assertAppraisalAccess(organizationId, appraisal, user, "MANAGER"); if (appraisal.stage === "SELF_ASSESSMENT") return appraisal; if (appraisal.stage !== "GOAL_SETTING") throw conflict("Goal setting is already locked"); if (!appraisal.goals.length) throw conflict("At least one appraisal goal is required before employee confirmation"); const notification = await notifyHRISUsers({ organizationId, recipientUserIds: await userIdsForEmployees(organizationId, [appraisal.employeeId]), categoryKey: "approvals-requests", eventKey: `appraisal:${appraisal.id}:goals:awaiting-confirmation`, type: "APPRAISAL_GOALS_AWAITING_CONFIRMATION", title: "Appraisal goals awaiting confirmation", message: `Your goals for ${appraisal.cycle.title} are ready for confirmation.`, metadata: { appraisalId: appraisal.id, employeeId: appraisal.employeeId } }); await createAuditLog({ organizationId, actorUserId: user.id, action: "HRIS_APPRAISAL_GOALS_READY_FOR_CONFIRMATION", resource: "APPRAISAL", resourceId: appraisal.id, summary: "Marked appraisal goals ready for employee confirmation", metadata: { employeeId: appraisal.employeeId, notification } }); return { ...appraisal, awaitingEmployeeConfirmation: true }; };
export const confirmEmployeeAppraisalGoals = async (organizationId: string, appraisalId: string, user: AuthUser) => {
  ...
  if (appraisal.goalsConfirmedAt && appraisal.stage !== "GOAL_SETTING") return appraisal;
  if (appraisal.stage !== "GOAL_SETTING") throw conflict("Goals cannot be confirmed at the current appraisal stage");
  if (!appraisal.goals.length) throw conflict("At least one appraisal goal is required before confirmation");
  const now = new Date(); const changed = await prisma.$transaction(async (tx) => { const advanced = await tx.employeeAppraisal.updateMany({ where: { id: appraisal.id, organizationId, employeeId: appraisal.employeeId, stage: "GOAL_SETTING", goalsConfirmedAt: null }, data: { goalsConfirmedAt: now, goalsConfirmedById: user.id, stage: "SELF_ASSESSMENT" } }); if (advanced.count === 1) await tx.appraisalGoal.updateMany({ where: { organizationId, appraisalId }, data: { status: "LOCKED" } }); return advanced; });
  if (changed.count !== 1) throw conflict("Appraisal goals were confirmed concurrently");
  const notification = await notifyHRISUsers({ organizationId, recipientUserIds: await userIdsForEmployees(organizationId, [appraisal.managerId]), categoryKey: "record-updates", eventKey: `appraisal:${appraisal.id}:goals:confirmed`, type: "APPRAISAL_GOALS_CONFIRMED", title: "Appraisal goals confirmed", message: `${appraisal.employee.firstName} ${appraisal.employee.lastName} confirmed the appraisal goals.`, metadata: { appraisalId, employeeId: appraisal.employeeId } });
  await createAuditLog({ organizationId, actorUserId: user.id, action: "HRIS_APPRAISAL_GOALS_CONFIRMED", resource: "APPRAISAL", resourceId: appraisal.id, summary: "Employee confirmed appraisal goals", metadata: { employeeId: appraisal.employeeId, notification } });
  return getTenantAppraisal(organizationId, appraisal.id);
};
...
export const appraisalAssessmentFromTemplate = (configuration: Prisma.JsonValue, submitted: any[]) => {
  const config = configuration && typeof configuration === "object" && !Array.isArray(configuration) ? configuration as Record<string, any> : {};
  const sections = Array.isArray(config.sections) ? config.sections : [];
  if (!sections.length || submitted.length !== sections.length) throw badRequest("Self-assessment must match the assigned appraisal template");
  return sections.map((section: any) => {
    const response = submitted.find((item: any) => item.section === section.section);
    if (!response || response.objectives.length !== section.objectives.length) throw badRequest("Self-assessment objectives do not match the assigned template");
    return {
      section: section.section,
      totalWeight: section.weight,
      objectives: section.objectives.map((objective: any, objectiveIndex: number) => {
        const answer = response.objectives[objectiveIndex];
        if (!answer || answer.title !== objective.title || answer.keyResults.length !== objective.keyResults.length) throw badRequest("Self-assessment objective definitions cannot be changed");
        return {
          title: objective.title,
          weight: objective.weight,
          keyResults: objective.keyResults.map((keyResult: any, keyResultIndex: number) => {
            const result = answer.keyResults[keyResultIndex];
            if (!result || result.keyResult !== keyResult.description || result.kpiWeight !== keyResult.kpiWeight || result.target !== keyResult.target) throw badRequest("Self-assessment KPI definitions cannot be changed");
            if (config.quarterScoring === false && typeof result.achieved === "object") throw badRequest("This appraisal template does not support quarterly achievement values");
            return { keyResult: keyResult.description, kpiWeight: keyResult.kpiWeight, target: keyResult.target, initiatives: keyResult.initiatives, achieved: result.achieved, comment: result.comment };
          })
        };
      })
    };
...
const hasAchievement = (achieved: unknown) => typeof achieved === "number" || Boolean(achieved && typeof achieved === "object" && Object.values(achieved).some((value) => value != null));
const hasMeaningfulSelfAssessmentInput = (sections: any[], reflections: any[]) => reflections.some((item) => item.response.trim().length > 0) || sections.some((section) => section.objectives.some((objective: any) => objective.keyResults.some((result: any) => hasAchievement(result.achieved))));
export const mergeAppraisalDraft = (incoming: any[], existing: any[] = []) => incoming.map((section, sectionIndex) => ({ ...section, objectives: section.objectives.map((objective: any, objectiveIndex: number) => ({ ...objective, keyResults: objective.keyResults.map((result: any, resultIndex: number) => { const previous = existing[sectionIndex]?.objectives?.[objectiveIndex]?.keyResults?.[resultIndex]; return { ...result, achieved: result.achieved === undefined ? previous?.achieved : result.achieved, comment: result.comment === undefined ? previous?.comment : result.comment }; }) })) }));
export const assertCompleteAppraisalSubmission = (sections: any[], reflections: any[], questions: any[]) => {
  if (sections.some((section) => section.objectives.some((objective: any) => objective.keyResults.some((result: any) => !hasAchievement(result.achieved))))) throw badRequest("Every appraisal KPI requires an achieved value before submission");
  const answered = new Set(reflections.filter((item) => item.response.trim()).map((item) => item.questionId));
  if (questions.some((question) => question.required !== false && !answered.has(String(question.id)))) throw badRequest("Every required self-assessment question requires a response before submission");
};
export const isAppraisalSubmissionDeadlineOpen = (deadline: Date | null, now: Date, timeZone: string) => !deadline || tenantDateKey(now, timeZone) <= tenantDateKey(deadline, timeZone);
...
export const saveSelfAssessment = async (organizationId: string, appraisalId: string, input: any, user: AuthUser) => {
  ...
  if (input.submit) { const settings = await prisma.organizationGeneralSettings.findUnique({ where: { organizationId }, select: { timeZone: true } }); if (!isAppraisalSubmissionDeadlineOpen(appraisal.cycle.deadline, new Date(), safeTimeZone(settings?.timeZone))) throw conflict("The self-assessment submission deadline has passed"); }
  const snapshot = appraisal.templateSnapshot ?? appraisal.cycle.templateSnapshot ?? appraisal.template.configuration; const submittedSections = appraisalAssessmentFromTemplate(snapshot, input.sections); const authoritativeSections = input.submit ? submittedSections : mergeAppraisalDraft(submittedSections, Array.isArray((appraisal.selfAssessment as any)?.sections) ? (appraisal.selfAssessment as any).sections : []); const weighted = calculateWeightedAssessmentScore(authoritativeSections); const questions = templateQuestions(snapshot); const validQuestionIds = new Set(questions.map((question: any) => String(question.id)));
  ...
  const reflections = input.submit ? input.reflections : [...((Array.isArray((appraisal.selfAssessment as any)?.reflections) ? (appraisal.selfAssessment as any).reflections : []) as any[]).filter((previous) => !input.reflections.some((current: any) => current.questionId === previous.questionId)), ...input.reflections];
  if (input.submit) { assertCompleteAppraisalSubmission(authoritativeSections, reflections, questions); if (!hasMeaningfulSelfAssessmentInput(authoritativeSections, reflections)) throw badRequest("Enter at least one achieved value or reflection answer before submitting"); }
  const result = await prisma.$transaction(async (tx) => {
    if (input.submit) { const advanced = await tx.employeeAppraisal.updateMany({ where: { id: appraisal.id, organizationId, employeeId: appraisal.employeeId, stage: "SELF_ASSESSMENT" }, data: { stage: "MANAGER_REVIEW", finalScore: weighted.score, rating: performanceRatingForScore(weighted.score), ratingValue: performanceRatingValueForScore(weighted.score) } }); if (advanced.count !== 1) throw conflict("Self-assessment was submitted concurrently"); }
    const assessment = await tx.appraisalSelfAssessment.upsert({ where: { appraisalId }, create: { organizationId, appraisalId, status: input.submit ? "SUBMITTED" : "IN_PROGRESS", sections: weighted.sections, reflections, submittedAt: input.submit ? new Date() : null }, update: { status: input.submit ? "SUBMITTED" : "IN_PROGRESS", sections: weighted.sections, reflections, submittedAt: input.submit ? new Date() : null } });
    if (input.submit) { await tx.appraisalGoal.updateMany({ where: { organizationId, appraisalId }, data: { status: "LOCKED" } }); await tx.appraisalSignOff.upsert({ where: { appraisalId_signOffType: { appraisalId, signOffType: "EMPLOYEE" } }, create: { organizationId, appraisalId, signatoryUserId: user.id, signOffType: "EMPLOYEE", signatoryRole: "EMPLOYEE" }, update: {} }); }
    return assessment;
...
export const getManagerReview = async (organizationId: string, appraisalId: string, user: AuthUser) => { const appraisal = await getTenantAppraisal(organizationId, appraisalId); await assertAppraisalAccess(organizationId, appraisal, user, "MANAGER"); if (appraisal.selfAssessment?.status !== "SUBMITTED" || !["MANAGER_REVIEW", "HR_APPROVAL", "ACKNOWLEDGMENT", "COMPLETED"].includes(appraisal.stage)) throw conflict("Employee self-assessment has not been submitted"); const snapshot = appraisal.templateSnapshot ?? appraisal.cycle.templateSnapshot ?? appraisal.template.configuration; return { appraisalId, stage: appraisal.stage, employee: { id: appraisal.employee.id, employeeCode: appraisal.employee.employeeNo, name: `${appraisal.employee.firstName} ${appraisal.employee.lastName}`, role: appraisal.employee.jobTitle, department: appraisal.employee.department?.name ?? null }, reviewer: appraisal.manager ? { id: appraisal.manager.id, name: `${appraisal.manager.firstName} ${appraisal.manager.lastName}`, role: appraisal.manager.jobTitle } : null, cycle: { id: appraisal.cycle.id, name: appraisal.cycle.title, periodFrom: appraisal.cycle.periodStart, periodTo: appraisal.cycle.periodEnd }, ratingScale: (snapshot as any)?.ratingScale ?? [], criteria: (snapshot as any)?.sections ?? [], employeeAssessment: appraisal.selfAssessment, goals: appraisal.goals, reflectionQuestions: templateQuestions(snapshot), managerReviewQuestions: templateManagerQuestions(snapshot), review: appraisal.managerReview }; };
export const saveManagerReview = async (organizationId: string, appraisalId: string, input: any, user: AuthUser) => {
  ...
  if (input.submit) { const ratedGoalIds = new Set(input.goalRatings.map((rating: any) => rating.goalId)); if (appraisal.goals.some((goal) => !ratedGoalIds.has(goal.id))) throw badRequest("Every appraisal goal requires a manager rating before submission"); const answeredQuestionIds = new Set(input.responses.map((response: any) => response.questionId)); const requiredManagerQuestions = templateManagerQuestions(snapshot).filter((question: any) => question.required !== false); if (requiredManagerQuestions.some((question: any) => !answeredQuestionIds.has(String(question.id)))) throw badRequest("Every required manager-review question requires a response before submission"); }
  const result = await prisma.$transaction(async (tx) => {
    await Promise.all(input.goalRatings.map((rating: any) => tx.appraisalGoal.update({ where: { id: rating.goalId }, data: { managerRating: rating.rating, managerComment: rating.comment } })));
...
export const createAppraisalTemplate = async (organizationId: string, input: any, user: AuthUser) => {
  ...
  const row = await prisma.$transaction(async (tx) => { await tx.$queryRaw`SELECT id FROM Organization WHERE id = ${organizationId} FOR UPDATE`; if (input.isDefault) await tx.appraisalTemplate.updateMany({ where: { organizationId, isDefault: true }, data: { isDefault: false, defaultKey: null } }); return tx.appraisalTemplate.create({ data: { organizationId, name: input.name, description: input.description, configuration, isDefault: input.isDefault, defaultKey: input.isDefault ? "DEFAULT" : null } }); });
...
export const updateAppraisalTemplate = async (organizationId: string, templateId: string, input: any, user: AuthUser) => {
  ...
  const updated = await prisma.$transaction(async (tx) => { await tx.$queryRaw`SELECT id FROM Organization WHERE id = ${organizationId} FOR UPDATE`; if (input.isDefault) await tx.appraisalTemplate.updateMany({ where: { organizationId, isDefault: true, id: { not: current.id } }, data: { isDefault: false, defaultKey: null } }); return tx.appraisalTemplate.update({ where: { id: current.id }, data: { name: input.name, description: input.description, configuration, isDefault: input.isDefault, defaultKey: input.isDefault ? "DEFAULT" : null, version: { increment: 1 } } }); });
...
export const appraisalTemplateSnapshot = (configuration: Prisma.JsonValue, template: { id: string; name: string; version: number }) => ({ ...(configuration && typeof configuration === "object" && !Array.isArray(configuration) ? configuration as Record<string, unknown> : {}), templateMetadata: { id: template.id, name: template.name, version: template.version } }) as Prisma.InputJsonValue;
export const createAppraisalCycle = async (organizationId: string, input: any, user: AuthUser) => {
  ...
  const snapshot = appraisalTemplateSnapshot(template.configuration, template); let enrolled = 0;
  const cycle = await prisma.$transaction(async (tx) => {
    await tx.$queryRaw`SELECT id FROM Organization WHERE id = ${organizationId} FOR UPDATE`;
    const periodStart = appraisalDate(input.periodFrom); const periodEnd = appraisalDate(input.periodTo);
    if (input.launchMode === "LAUNCH_AS_ACTIVE" && await tx.appraisalCycle.findFirst({ where: { organizationId, status: "OPEN", periodStart: { lte: periodEnd }, periodEnd: { gte: periodStart } }, select: { id: true } })) throw conflict("An overlapping active appraisal cycle already exists");
    const created = await tx.appraisalCycle.create({ data: { organizationId, title: input.cycleName, description: input.description, periodStart, periodEnd, deadline: appraisalDate(input.submissionDeadline), quarter: quarterForDate(periodStart), year: periodStart.getUTCFullYear(), templateId: template.id, templateSnapshot: snapshot, status: input.launchMode === "LAUNCH_AS_ACTIVE" ? "OPEN" : "DRAFT" } });
    if (input.launchMode === "LAUNCH_AS_ACTIVE") enrolled = await launchCycleTx(tx, organizationId, created.id, template.id, snapshot, user.id); return created;
  });
...
export const launchAppraisalCycle = async (organizationId: string, cycleId: string, user: AuthUser) => { assertAppraisalAdministrator(user); const cycle = await prisma.appraisalCycle.findFirst({ where: { id: cycleId, organizationId }, include: { template: true } }); if (!cycle) throw notFound("Appraisal cycle not found"); if (cycle.status === "OPEN") return { ...cycle, status: "ACTIVE", enrolledEmployees: await prisma.employeeAppraisal.count({ where: { organizationId, cycleId } }) }; if (cycle.status !== "DRAFT" || !cycle.template || cycle.template.archivedAt) throw conflict("Only a draft cycle with an active template can be launched"); const baseSnapshot = cycle.templateSnapshot ?? cycle.template.configuration; const snapshot = baseSnapshot && typeof baseSnapshot === "object" && !Array.isArray(baseSnapshot) && "templateMetadata" in baseSnapshot ? baseSnapshot as Prisma.InputJsonValue : appraisalTemplateSnapshot(baseSnapshot, cycle.template); const enrolledEmployees = await prisma.$transaction(async (tx) => { await tx.$queryRaw`SELECT id FROM Organization WHERE id = ${organizationId} FOR UPDATE`; if (await tx.appraisalCycle.findFirst({ where: { organizationId, status: "OPEN", id: { not: cycle.id }, periodStart: { lte: cycle.periodEnd }, periodEnd: { gte: cycle.periodStart } }, select: { id: true } })) throw conflict("An overlapping active appraisal cycle already exists"); return launchCycleTx(tx, organizationId, cycle.id, cycle.template!.id, snapshot, user.id); }); await createAuditLog({ organizationId, actorUserId: user.id, action: "HRIS_APPRAISAL_CYCLE_LAUNCHED", resource: "APPRAISAL_CYCLE", resourceId: cycle.id, summary: `Launched appraisal cycle ${cycle.title}`, metadata: { enrolledEmployees, templateVersion: cycle.template.version } }); return { ...cycle, status: "ACTIVE", enrolledEmployees };
};
...
export const deleteAppraisalCycle = async (organizationId: string, cycleId: string, user: AuthUser) => {
  assertAppraisalAdministrator(user);
  const cycle = await prisma.appraisalCycle.findFirst({ where: { id: cycleId, organizationId }, select: { id: true, title: true, status: true } });
  if (!cycle) throw notFound("Appraisal cycle not found");
  if (cycle.status !== "DRAFT") throw conflict("Only an empty draft appraisal cycle can be permanently deleted");
  const appraisalCount = await prisma.employeeAppraisal.count({ where: { organizationId, cycleId } });
  if (appraisalCount) throw conflict("An appraisal cycle with employee history cannot be permanently deleted");
  await prisma.appraisalCycle.delete({ where: { id: cycle.id } });
  await createAuditLog({ organizationId, actorUserId: user.id, action: "HRIS_APPRAISAL_CYCLE_DELETED", resource: "APPRAISAL_CYCLE", resourceId: cycle.id, summary: `Permanently deleted empty draft appraisal cycle ${cycle.title}`, metadata: { previousStatus: cycle.status } });
  return { id: cycle.id, deleted: true, deletedAppraisals: 0 };
};
