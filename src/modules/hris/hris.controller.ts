...
import { downloadManagedEmployeeDocument } from "./hris.service";
...
export const downloadManagedEmployeeDocumentController = async (req: Request, res: Response) => { const document = await downloadManagedEmployeeDocument(req.organizationId!, String(req.params.employeeId), String(req.params.documentId), req.user!); return res.status(200).type(document.mimeType).attachment(document.filename).send(document.buffer); };
