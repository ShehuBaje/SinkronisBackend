...
export const getPlatformAnalytics = async (input: unknown, user: AuthUser) => {
  ...
  const organizations = await prisma.organization.findMany({ where: { status: { not: "ARCHIVED" }, users: { none: { isPlatformAdmin: true } } }, select: { id: true, name: true, status: true, createdAt: true } });
  ...
  const [subscriptionRows, userRows, sessionRows, billingRows, moduleRows, usageRows] = await Promise.all([
    tenantIds.length ? prisma.systemConfig.findMany({ where: { organizationId: { in: tenantIds }, key: "billing.subscription" }, select: { organizationId: true, value: true } }) : [],
    tenantIds.length ? prisma.user.findMany({ where: { organizationId: { in: tenantIds } }, select: { organizationId: true, lastLoginAt: true } }) : [],
    tenantIds.length ? prisma.userSession.findMany({ where: { organizationId: { in: tenantIds } }, select: { organizationId: true, createdAt: true, lastSeenAt: true } }) : [],
    tenantIds.length ? prisma.billingHistory.findMany({ where: { organizationId: { in: tenantIds }, billedAt: { gte: range.from, lt: until }, status: { in: ["PAID", "COMPLETED", "SUCCESS"] } }, select: { organizationId: true, billedAt: true, amount: true, metadata: true } }) : [],
  ...
  const subscriptions = new Map(subscriptionRows.map((row) => [row.organizationId, row.value && typeof row.value === "object" && !Array.isArray(row.value) ? row.value as Record<string, unknown> : {}]));
  const subscription = (tenantId: string) => subscriptions.get(tenantId) ?? {};
  for (const tenant of organizations) {
    ...
    const config = subscription(tenant.id); const lifecycleDate = new Date(String(config.cancelledAt ?? config.expiredAt ?? config.renewalDate ?? "")); const plan = typeof config.planKey === "string" ? config.planKey : null; if (plan && ["CANCELLED", "EXPIRED"].includes(String(config.status ?? "").toUpperCase()) && !Number.isNaN(lifecycleDate.getTime()) && lifecycleDate >= range.from && lifecycleDate < until) churnByPlan.set(plan, (churnByPlan.get(plan) ?? 0) + 1);
  }
  ...
  const activity = organizations.map((tenant) => { const users = userRows.filter((row) => row.organizationId === tenant.id); const sessions = sessionRows.filter((row) => row.organizationId === tenant.id); const usage = usageRows.filter((row) => row.organizationId === tenant.id); const sessionsInRange = sessions.filter((session) => session.createdAt >= range.from && session.createdAt < until).length; const pagesVisited = usage.filter((row) => row.usageDate >= range.from && row.usageDate < until).reduce((sum, row) => sum + row.pageViews, 0); const dates = [...users.map((item) => item.lastLoginAt), ...sessions.map((item) => item.lastSeenAt), ...usage.map((item) => item.lastActivityAt)].filter((item): item is Date => Boolean(item)); const lastActive = dates.sort((a, b) => b.getTime() - a.getTime())[0] ?? null; const lastLogin = users.map((item) => item.lastLoginAt).filter((item): item is Date => Boolean(item)).sort((a, b) => b.getTime() - a.getTime())[0] ?? null; const config = subscription(tenant.id); return { tenantId: tenant.id, company: tenant.name, createdAt: tenant.createdAt, status: tenant.status, plan: typeof config.planKey === "string" ? String(config.planKey) : null, lastLogin, sessionsThisMonth: sessionsInRange, pagesVisited, lastActive }; });
